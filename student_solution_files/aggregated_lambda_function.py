...
from botocore.exceptions import ClientError  # Exception handling for AWS services


# ** key phrases function **
def key_phrase_finder(list_of_important_phrases, list_of_extracted_phrases):
    """
    This function attempts to find a match between the words in a key phrases dictionary
    produced by AWS Comprehend to a provided custom list of words. The result of the search
    is returned as a boolean variable.

    Input arguments:
    list_of_important_phrases: A string-based list of words representing topics to be detected
                                i.e. ['CV', 'data', 'science']
    list_of_extracted_phrases: An AWS Comprehend key phrases dictionary.

    Outputs:
    listing: An empty list to append the individual words present in the AWS Comprehend
            key phrases dictionary.
    phrase_checker: A boolean variable representing whether a match is discovered
                    between the function's input lists.
    """
    listing = []
    PhraseChecker = None
...
def email_response(name, critical_phrase_list, list_of_extracted_phrases, AWS_Comprehend_Sentiment_Dump):
    """
    This function takes in the parsed information from the sender i.e. name, email_address,
    the sentiment and the key phrases output from AWS Comprehend, and a list of critical phrases,
    and uses the logic described in the 'Process Description' section to populate an email.
...
def lambda_handler(event, context):
    ...
    try:
        # Generating a unique ID for the response
        response_id = str(uuid.uuid4())

        # Writing to DynamoDB
        db_response = table.put_item(
            Item={
                'ResponsesID': response_id,
                'Name': dec_dict['name'],
    ...
    try:
        ...
        return {
            'statusCode': 500,
            'body': json.dumps('Error with AWS Comprehend')
        }
    # -----------------------------

    # Get list of phrases in numpy array
    phrase = []
    for i in range(0, len(key_phrases['KeyPhrases'])):
    ...
    # --- Insert your code here ---
    email_text = email_response(dec_dict['name'], ['CV', 'data', 'science'], phrase, sentiment)
